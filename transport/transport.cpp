...
string Transport::get_yaml_conf_path(){
    return string("./conf/site_deploy.yml");
}
...
Socket *Transport::connect(uint64_t dest_id, uint64_t port_id) {
    ...
    //  sprintf(socket_name,"tcp://eth0;%s:%ld",ifaddr[dest_id],port_id);
      sprintf(socket_name,"tcp://%s:%ld",ifaddr[dest_id],port_id);
#else
...
void Transport::init() {
    ...
    for (uint64_t node_id = 0; node_id < g_total_node_cnt; node_id++) {
        ...
        if (ISCLIENTN(node_id)) {
            for (uint64_t client_thread_id = g_client_thread_cnt + g_client_rem_thread_cnt; client_thread_id <
                                                                                            g_client_thread_cnt +
                                                                                            g_client_rem_thread_cnt +
                                                                                            g_client_send_thread_cnt; client_thread_id++) {
            ...
            for (uint64_t server_thread_id = g_thread_cnt + g_rem_thread_cnt;
                 server_thread_id < g_thread_cnt + g_rem_thread_cnt + g_send_thread_cnt; server_thread_id++) {
