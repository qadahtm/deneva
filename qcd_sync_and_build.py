...
def enum_exps(seq, YCSB_ENABLED=False, TPCC_ENABLED=False):
    all_algs = [  # 'NO_WAIT',
        'CALVIN',
        # 'MVCC',
        # 'MAAT',
                            ...
                            for ppt in ppt_vals:
                                for rpq in req_per_query_vals:
                                    ...
                                    for bs in batch_size_vals:
                                        ...
                                        ycsb.update(
                                            {'read_ratio': read_ratio, 'zipf_theta': zipf_theta, 'req_per_query': rpq,
                                             'batch_size': bs})
...
def sort_exps(exps):
    def _exp_pri(exp):
        ...
        if exp['bench'].startswith('TPCC') and exp['warehouse_count'] in (
                1, 4, 16, max_thread_count, max_thread_count * 2): pri -= 1
...
def run_all_seq(pats, prepare_only):
    ...
    if not prepare_only:
        setup_node_list(os.environ.get('IFCONFIG_FILE_PATH'))
...
def exec_cmd(cmd, env, async_cmd=False):
...
def setup_node_list(ifconfig_path):
    ...
    for nip in node_list:
        ...
        rcmd = 'scp -o StrictHostKeyChecking=no /mnt/efs/expodb/ifconfig.txt ubuntu@{}:~/{}/ifconfig.txt'.format(nip,
                                                                                                                 build_dir)
...
def build_executables():
    ...
    for nip in node_list:
        ...
        rcmd = 'ssh -oStrictHostKeyChecking=no ubuntu@{} "{}"'.format(nip,
                                                                      'cd ~/{}; make clean; make -j8'.format(build_dir))
        proc_list.append(exec_cmd(rcmd, env, True))
    wait_for(proc_list)
    proc_list.clear()
    print('done (building)!')
        ...
        for thd in thd_list:
            if thd:
                ...
                if thd.returncode != 0 or thd.killed:
                    ...
                    error_s = error_s + ('failed to run exp for %s (status=%s, killed=%s)' % (
                        format_exp(exp), thd.returncode, thd.killed))
                    print(COLOR_RED + error_s + COLOR_RESET)
                    failed = True
...
with open(os.environ.get('SECRETS_FILE')) as data_file:
...
res_dir_name = '/Users/qadahtm/Dev/expodb_results'
