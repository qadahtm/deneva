...
TEST(ConfigYaml, LoadTest_NoZks){
    ...
    ASSERT_EQ(conf->replicas->at(0).size(), 1);
    ASSERT_EQ(conf->replicas->at(1).size(), 1);
...
TEST(ConfigYaml, LoadTest_YamlCpp){
    ...
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(conf->servers->size(), 6);
    ASSERT_STREQ(conf->servers->at(0).c_str(), "192.168.1.200");
    ASSERT_STREQ(conf->servers->at(1).c_str(), "192.168.1.1");
    ASSERT_STREQ(conf->servers->at(2).c_str(), "192.168.1.2");
    ASSERT_STREQ(conf->servers->at(3).c_str(), "192.168.2.200");
    ASSERT_STREQ(conf->servers->at(4).c_str(), "192.168.2.100");
    ASSERT_STREQ(conf->servers->at(5).c_str(), "192.168.2.101");

    ASSERT_EQ(conf->replicas->at(0).size(), 2);
    ASSERT_EQ(conf->replicas->at(0).at(0), 1);
    ASSERT_EQ(conf->replicas->at(0).at(1), 2);

    ASSERT_EQ(conf->replicas->at(1).size(), 2);
    ASSERT_EQ(conf->replicas->at(1).at(0),  0);
    ASSERT_EQ(conf->replicas->at(1).at(1), 2);
