...
//	}
//
//	// TODO(tq): store only one item for now. We may want to remove the key as well.
//	idx_key_t 		key;
//	itemid_t item;
//};
...
class IndexHashSimple : public index_base {
public:
	RC init(uint64_t bucket_cnt);
...
//	bool get_latch(BucketHeader * bucket, latch_t latch_type);
//	bool release_latch(BucketHeader * bucket, latch_t latch_type);
	void get_latch(BucketHeaderSimple *bucket);

	void release_latch(BucketHeaderSimple *bucket);
