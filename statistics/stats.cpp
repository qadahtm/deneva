...
void Stats_thd::print(FILE * outf, bool prog) {
  ...
  for(uint64_t i = 0; i < g_plan_thread_cnt; i ++) {
    ...
    fprintf(outf,
            ",quecc_plan%ld_queue_deq_cnt=%ld"
            ,i
    ...
    fprintf(outf,
            ",quecc_plan%ld_queue_deq_pop_time=%f"
            ,i
            ,plan_queue_deq_pop_time[i] /BILLION
    );
    fprintf(outf,
            ",quecc_plan%ld_queue_deq_free_mem_time=%f"
            ,i
            ,plan_queue_deq_free_mem_time[i] /BILLION
    );
    fprintf(outf,
            ",quecc_plan%ld_batch_delivery_time=%f"
            ,i
            ,plan_batch_delivery_time[i] /BILLION
    );
    fprintf(outf,
            ",quecc_plan%ld_txn_proc_time=%f"
            ,i
            ,plan_txn_process_time[i] /BILLION
    );
    fprintf(outf,
            ",quecc_plan%ld_total_time=%f"
            ,i
            ,plan_total_time[i] /BILLION
    );
  }
  fprintf(outf,
  ...
  for(uint64_t i = 0; i < g_thread_cnt; i ++) {
    ...
    fprintf(outf,
            ",quecc_exec%ld_txn_frag_cnt=%ld"
            ,i
            ,exec_txn_frag_cnt[i]
    );
    fprintf(outf,
            ",quecc_exec%ld_batch_part_cnt=%ld"
            ,i
            ,exec_batch_part_cnt[i]
    );
    fprintf(outf,
            ",quecc_exec%ld_batch_cnt=%ld"
            ,i
            ,exec_batch_cnt[i]
    );
    fprintf(outf,
    ...
    fprintf(outf,
            ",quecc_exec%ld_mem_free_time=%f"
            ,i
            ,exec_mem_free_time[i] /BILLION
    );
    fprintf(outf,
            ",quecc_exec%ld_idle_time=%f"
            ,i
    ...
    fprintf(outf,
            ",quecc_exec%ld_entry_deq_time=%f"
            ,i
            ,exec_entry_deq_time[i] /BILLION
    );
    fprintf(outf,
            ",quecc_exec%ld_txn_wait_time=%f"
            ,i
