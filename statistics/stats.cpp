...
// QueCC or LADS
    ...
    plan_batch_size_sent_total = (double *) mem_allocator.align_alloc(sizeof(double) * g_plan_thread_cnt);
    ...
    plan_total_time = (double *) mem_allocator.align_alloc(sizeof(double) * g_plan_thread_cnt);
...
void Stats_thd::clear() {
    ...
    for (uint64_t i = 0; i < g_plan_thread_cnt; i++) {
        ...
        plan_queue_deq_cnt[i] = 0;
        plan_batch_size_sent_total[i] = 0;
...
//#endif
    ...
    fprintf(outf,
            ...
            ",txn_run_time=%f"
            ",txn_run_avg_time=%f"
            ",cl_send_intv=%f", total_runtime / BILLION
//#if CC_ALG == QUECC
...
//          ,quecc_tput
//#endif
            , tput, txn_cnt, txn_sent_cnt
#if ABORT_MODE
            ,total_txn_abort_cnt
#endif
            , txn_run_time / BILLION, txn_run_avg_time / BILLION, cl_send_intv / BILLION
    );
    // IO
    ...
    fprintf(outf,
            ...
            ",msg_copy_output_time=%f", msg_queue_delay_time / BILLION, msg_queue_cnt, msg_queue_enq_cnt,
            msg_queue_delay_time_avg / BILLION, msg_send_time / BILLION, msg_send_time_avg / BILLION,
            msg_recv_time / BILLION, msg_recv_time_avg / BILLION, msg_recv_idle_time / BILLION, msg_batch_cnt,
            msg_batch_size_msgs, msg_batch_size_msgs_avg, msg_batch_size_bytes, msg_batch_size_bytes_avg,
            msg_batch_size_bytes_to_server, msg_batch_size_bytes_to_client, msg_send_cnt, msg_recv_cnt,
            msg_unpack_time / BILLION, msg_unpack_time_avg / BILLION, mbuf_send_intv_time / BILLION,
            mbuf_send_intv_time_avg / BILLION, msg_copy_output_time / BILLION
    );
    ...
    if (!prog) {
        ...
        fprintf(outf,
                ...
                ",ccl98=%f"
                ",ccl99=%f"
                ",ccl100=%f", (double) client_client_latency.get_idx(0) / BILLION,
...
void Stats_thd::print(FILE *outf, bool prog) {
    fprintf(outf,
            "total_runtime=%f", total_runtime / BILLION
    );
    // Execution
    double tput = 0;
    double txn_run_avg_time = 0;
    ...
    fprintf(outf,
            ...
            local_txn_commit_cnt, remote_txn_commit_cnt, total_txn_abort_cnt, unique_txn_abort_cnt, local_txn_abort_cnt,
            remote_txn_abort_cnt
#if ABORT_MODE
            ,txn_dd_abort_cnt
#endif
            , txn_run_time / BILLION, txn_run_avg_time / BILLION, multi_part_txn_cnt, multi_part_txn_run_time / BILLION,
            multi_part_txn_avg_time / BILLION, single_part_txn_cnt, single_part_txn_run_time / BILLION,
            single_part_txn_avg_time / BILLION, txn_write_cnt, record_write_cnt, parts_touched, avg_parts_touched
    );
    ...
    for (uint64_t i = 0; i < g_thread_cnt; i++) {
        ...
        fprintf(outf,
                ",wt%ld_record_copy_time_s=%f", i, record_copy_time[i] / BILLION);
    }
    fprintf(outf,
            ",total_record_copy_time_ns=%f", (total_record_copy_time));
    fprintf(outf,
            ",avg_record_copy_time_ns=%f", (total_record_copy_time / (total_record_copy_cnt)));
    fprintf(outf,
            ",total_record_copy_cnt=%ld", total_record_copy_cnt);
    fprintf(outf,
            ",total_record_recov_cnt=%ld", total_record_recov_cnt);
    ...
    for (uint64_t i = 0; i < g_plan_thread_cnt; i++) {
        fprintf(outf,
                ",quecc_plan%ld_txn_cnt=%ld", i, plan_txn_cnts[i]
        );
        fprintf(outf,
                ",quecc_plan%ld_batch_cnt=%ld", i, plan_batch_cnts[i]
        );
        fprintf(outf,
                ",quecc_plan%ld_size_batch_cnt=%ld", i, plan_size_batch_cnts[i]
        );
        fprintf(outf,
                ",quecc_plan%ld_time_batch_cnt=%ld", i, plan_time_batch_cnts[i]
        );
#if PROFILE_EXEC_TIMING
        fprintf(outf,
                ",quecc_plan%ld_mem_alloc_time=%f", i, plan_mem_alloc_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_idle_time=%f", i // idle time as measured includes dequeue time so
                , (plan_idle_time[i]) / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_split_time=%f", i // idle time as measured includes dequeue time so
                , (plan_split_time[i]) / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_merge_time=%f", i // idle time as measured includes dequeue time so
                , (plan_merge_time[i]) / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_tdep_time=%f", i // idle time as measured includes dequeue time so
                , (plan_tdep_time[i]) / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_batch_proc_time=%f", i, plan_batch_process_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_queue_dequeue_time=%f", i, plan_queue_dequeue_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_queue_deq_pop_time=%f", i, plan_queue_deq_pop_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_queue_deq_free_mem_time=%f", i, plan_queue_deq_free_mem_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_batch_delivery_time=%f", i, plan_batch_delivery_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_txn_proc_time=%f", i, plan_txn_process_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_plan%ld_total_time=%f", i, plan_total_time[i] / BILLION
        );
#endif // #if PROFILE_EXEC_TIMING

        fprintf(outf,
                ",quecc_plan%ld_queue_deq_cnt=%ld", i, plan_queue_deq_cnt[i]
        );
        fprintf(outf,
                ",quecc_plan%ld_reuse_exec_queue_cnt=%ld", i, plan_reuse_exec_queue_cnt[i]
        );
        fprintf(outf,
                ",quecc_plan%ld_alloc_exec_queue_cnt=%ld", i, plan_alloc_exec_queue_cnt[i]
        );
        fprintf(outf,
                ",quecc_plan%ld_avg_batch_size(KB)=%f", i,
                plan_batch_size_sent_total[i] / ((double) (1000 * plan_batch_cnts[i]))
        );
    }

#if PROFILE_EXEC_TIMING
    fprintf(outf,
            ",quecc_plan_queue_wait_time=%f", plan_queue_wait_time / BILLION
    );
#endif

    for (uint64_t i = 0; i < g_thread_cnt; i++) {
        fprintf(outf,
                ",quecc_exec%ld_txn_cnt=%ld", i, exec_txn_cnts[i]
        );
        fprintf(outf,
                ",quecc_exec%ld_txn_frag_cnt=%ld", i, exec_txn_frag_cnt[i]
        );
        fprintf(outf,
                ",quecc_exec%ld_batch_part_cnt=%ld", i, exec_batch_part_cnt[i]
        );
        fprintf(outf,
                ",quecc_exec%ld_batch_cnt=%ld", i, exec_batch_cnt[i]
        );
#if PROFILE_EXEC_TIMING
        fprintf(outf,
                ",quecc_exec%ld_batch_proc_time=%f", i, exec_batch_proc_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_avg_exec_%ld_batch_proc_time=%f", i, exec_batch_proc_time[i] / (BILLION * exec_batch_cnt[i])
        );
        fprintf(outf,
                ",quecc_exec%ld_mem_free_time=%f", i, exec_mem_free_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_idle_time=%f", i, exec_idle_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_resp_msg_create_time=%f", i, exec_resp_msg_create_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_batch_part_proc_time=%f", i, exec_batch_part_proc_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_txn_commit_time=%f", i, exec_txn_commit_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_txn_index_lookup_time=%f", i, exec_txn_index_lookup_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_txn_proc_time=%f", i, exec_txn_proc_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_entry_deq_time=%f", i, exec_entry_deq_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_txn_wait_time=%f", i, exec_txn_wait_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_exec%ld_eq_swtich_time=%f", i, exec_eq_swtich_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_hl_plan_time=%f", i, wt_hl_plan_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_hl_exec_time=%f", i, wt_hl_exec_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_hl_commit_time=%f", i, wt_hl_commit_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_hl_cleanup_time=%f", i, wt_hl_cleanup_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_hl_sync_plan_time=%f", i, wt_hl_sync_plan_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_hl_sync_exec_time=%f", i, wt_hl_sync_exec_time[i] / BILLION
        );
        fprintf(outf,
                ",quecc_wt%ld_pg_sync_exec_time=%f", i, wt_pg_sync_exec_time[i] / BILLION
        );
        fprintf(outf,
    ...
    if (!prog) {
        ...
        fprintf(outf,
                ...
                ",fscl100=%f"
                ",fscl_avg=%f"
                ",fscl_cnt=%ld", (double) first_start_commit_latency.get_idx(0) / BILLION,
        ...
        fprintf(outf,
                ...
                ",lscl100=%f"
                ",lscl_avg=%f"
                ",lscl_cnt=%ld", (double) last_start_commit_latency.get_idx(0) / BILLION,
        ...
        fprintf(outf,
                ...
                ",sacl100=%f"
                ",sacl_avg=%f"
                ",sacl_cnt=%ld", (double) start_abort_commit_latency.get_idx(0) / BILLION,
...
void Stats_thd::combine(Stats_thd *stats) {
    ...
    for (uint64_t i = 0; i < g_plan_thread_cnt; i++) {
        ...
        plan_queue_deq_cnt[i] += stats->plan_queue_deq_cnt[i];
        plan_batch_size_sent_total[i] += stats->plan_batch_size_sent_total[i];
        ...
        plan_txn_process_time[i] += stats->plan_txn_process_time[i];
...
// Physical memory used by current process, in KB
    while (fgets(line, 128, file) != NULL) {
        if (strncmp(line, "VmRSS:", 6) == 0) {
            ...
            fprintf(outf,
                    ",phys_mem_usage=%d", result
            );
        }
        if (strncmp(line, "VmSize:", 7) == 0) {
            result = parseLine(line);
            fprintf(outf,
                    ",virt_mem_usage=%d", result
            );
        }
    }
    fclose(file);
